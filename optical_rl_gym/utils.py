...
class PhysicalParameters:
    ...
    #alpha: float = 0.046051701859880924,  # 1/km   
    alpha: float = 4.6051701859880924e-2
    gamma: float = 1.2             
    beta_2: float = -21.27e-27     
    beta_3: float = 0.14e-39       
    nse: float = 1.5             
    cr: float = 2.8e-15   #0.028 1/W/km/THz
...
class Path:
    ...
    link_idx: List[int] = field(default_factory=list)  # List of link indices
    ...
    def __str__(self):
        ...
        links_str = '\n\t\t'.join([f"Link {i}: {link.node1}->{link.node2}, length={link.length:.2f}km"
         ...
         # Format link indices
        indices_str = ', '.join(map(str, self.link_idx))
...
class Service:
    ...
    modulation_format: Optional[str] = field(default=None)  # e.g., 'QPSK', '16QAM'
    band: Optional[int] = field(default=None)             # 0 for C-band, 1 for L-band
    initial_slot: Optional[int] = field(default=None)      # Starting slot
    number_slots: Optional[int] = field(default=None)      # Number of slots used
    center_frequency: Optional[float] = field(default=None) # Center frequency of allocation
    termination_time: Optional[float] = field(default=None) # When service will terminate
    current_OSNR: Optional[float] = field(default=None)   # in dB
    OSNR_th: Optional[float] = field(default=None)
    bandwidth: Optional[float] = field(default=None) #in GHz
    OSNR_margin: Optional[float] =field(default=None)
